...
macro_rules! forward_val_ref_binop_to_val_assign {
    (impl $imp:ident for $res:ty, $method:ident, $assign:ident, $assign_method:ident) => {
        impl<'a> $imp<&'a $res> for $res {
            type Output = $res;

            #[inline]
            fn $method(mut self, other: &$res) -> $res {
                // forward to assign trait
                $assign::$assign_method(&mut self, other);
                self
            }
        }
    };
...
macro_rules! impl_val_assign {
    (impl $imp:ident for $res:ty, $method:ident) => {
        impl<'a> $imp<&'a $res> for $res {
            #[inline]
            fn $method(&mut self, other: &$res) {
                check_moduli!(self, other);
                $imp::$method(&mut self.val, &other.val);
                ::std::ops::RemAssign::rem_assign(&mut self.val, &self.modulus);
            }
        }
    };
}

macro_rules! impl_all_binop_assign {
    (impl $imp:ident, $imp_assign:ident for $res:ty, $method:ident, $method_assign:ident) => {
        forward_all_binop_to_val_ref!(impl $imp for $res, $method);
        forward_val_ref_binop_to_val_assign!(impl $imp for $res, $method, $imp_assign, $method_assign);
        forward_val_assign!(impl $imp_assign for $res, $method_assign);
        impl_val_assign!(impl $imp_assign for $res, $method_assign);
    };
}
